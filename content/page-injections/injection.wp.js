          ...
          _this.request('name:entry?').then(function (granted) {
            if (granted) {
              ...
              if (_store.default.name) _this.retrieveEntry().then(function (entry) {
                ...
                //Saama custom function start
                var global_default_class = "";
                browser.storage.local.get().then(function (localStorage) {

                  var s_name = '(By: '+localStorage.smaccountusername+') '+entry.name;
                  s_name = s_name.substring(0, 70 - 1);
                  var counter = getDoubleNameCount(localStorage.history, s_name);
                  if (counter) s_name += ' (' + (counter + 1) + ')';
                  entry.name = s_name;


                  if(localStorage.smaccountsetting && localStorage.smaccountsetting == "Teacher"){
                    if(localStorage.smsharingenable && localStorage.smsharingenable == "Yes") {
                      if(localStorage.smdefaultclass) {
                        global_default_class = localStorage.smdefaultclass;
                        var classes_array = [];
                        if(entry.classes) {
                          classes_array = entry.classes;
                        }
                        classes_array.push(global_default_class);
                        entry.classes = classes_array;
                        saamaShareFunction(entry,'update');
          ...
          if (_store.default.isNew) {
            ...
            if (confirmed) {
              ...
              browser.storage.local.get().then(function (localStorage) {
                ...
                if(naming == "date") {
                  var s_name = new Date(entry.first).toLocaleString();
                  s_name = s_name.substring(0, 70 - 1);
                  var counter = getDoubleNameCount(localStorage.history, s_name);
                  if (counter) s_name += ' (' + (counter + 1) + ')';
                  entry.name = '(By: '+localStorage.smaccountusername+') '+s_name;
                }

                if(naming == "title") {
                  var s_name = entry.title;
                  s_name = s_name.substring(0, 70 - 1);
                  entry.name = '(By: '+localStorage.smaccountusername+') '+s_name;
                }
...
function saamaShareFunction(entry,action) {
...
function getDoubleNameCount(history, name) {
...
function isDoubleName(name, checkpoint) {
...
if(url.split('?')[0] == "http://saamaweb.com/classroom-extension/") {
  var params = '?';
  var index = url.indexOf(params);
  if (index > -1) {
    params = url.substring(index);
  }
  url = browser.extension.getURL("/content/addon-page/oauth.html" + params);
