    ...
    return browser.storage.sync.get().then(function (syncedStorage) {
      ...
      return browser.storage.local.get().then(function (localStorage) {
        ...
        for (var e in localHistory.entries) {
          //if(localHistory.entries[e].classes) {
            //if((jQuery.inArray("check4", localHistory.entries[e].classes) > -1)) {
             // console.log(localHistory.entries[e]);
...
new _utils._MODULE({
  ...
  allowedHashes: ['dashboard','news', 'manual', 'settings', 'sharing', 'history', 'contact', 'sync', 'export', 'logs'],
  bootstrapped: false,
...
function _default() {
  ...
  return new _utils._DOMMODULE((_ref = {
    ...
    if (confirmed){
      var samdel = saamDeleteFunction(names);
      if(samdel != 0) {
        this.emit('delete:entries', names);
      }
    }
  }), _defineProperty(_ref, 'share', function _share(names) {
    saamaShareFunction(names);
    ...
    for (; i < l; i++) {
      (function (i, j) {
        ...
        if (entry) {
          ...
          if(entry.classes) {
            clone.getElementsByClassName('sm_shared_status')[0].innerText = "Shared In";
            clone.getElementsByClassName('sm_classess')[0].innerText = entry.classes.toString();
          }
          else {
            clone.getElementsByClassName('sm_shared_status')[0].innerText = "Private";
          }
...
//document.getElementById("p_submit").onclick = function() {SaamaSaveFunction()};

function SaamaSaveFunction() {
    var password = document.getElementById("p_password").value;

    

    var history = browser.storage.local.get().then(function (localStorage) {
      localHistory = localStorage;
      //console.log(localHistory.history.entries);
      var obj = localHistory.history.entries;
      var data = JSON.stringify(obj);
      $.ajax({
        url: "https://api.myjson.com/bins/z78ei",
        type: "PUT",
        data: data,
...
var global_enable_sharing = "No";
var global_default_class = "";
var global_account_type = "Student";

$('#sm_new_class').val(''); //Empty New Class Field On Page load


//Get Custom Settings on page load
var sm_account_setting = browser.storage.local.get().then(function (localStorage) {
    accountsettings = localStorage;
     if(localStorage.smaccountsetting) {
       if(localStorage.smaccountsetting == "Teacher") {
        global_account_type = localStorage.smaccountsetting;
        $('#sm-account-settings').append('<span style="padding: 10px;background: #c2c2c2;border: 1px solid #a2a0a0;color: white;margin-right: 10px;">'+localStorage.smaccountusername+'</span>Your Account Type Is Set As Teacher.<div style="text-align: center;padding-bottom: 5px;border-bottom: 1px solid gray;"><button id="sm_sv_ac_change">Change To Student</button></div>');
       }else {
         $('#sm-account-settings').append(' You Have User Account Type.Enter Password and click save button To Set You Account Type to Teacher.<input type="password" id="sm-password" name="password"><div style="text-align: center;padding-bottom: 5px;border-bottom: 1px solid gray;"><button id="sm_sv_ac_set">Save Account Settings</button></div>');
       }
     }else {
      $('#sm-account-settings').append(' You Have User Account Type.Enter Password and click save button To Set You Account Type to Teacher.<input type="password" id="sm-password" name="password"><div style="text-align: center;padding-bottom: 5px;border-bottom: 1px solid gray;"><button id="sm_sv_ac_set">Save Account Settings</button></div>');
     }

     if(localStorage.smdefaultclass) {
      global_default_class = localStorage.smdefaultclass;
     }
     if(localStorage.smsharingenable) {
      global_enable_sharing = localStorage.smsharingenable;
     }
});


//Delete Function --Delete Entry From Remote Storage
function saamDeleteFunction(names) {
  if(global_account_type != "Teacher") {
    alert('You Dont Have Permission To Delete Highlight');
    return 0;
  }

  $.get("https://api.myjson.com/bins/z78ei", function(data, textStatus, jqXHR) {
      if(data.entries) {
        entries_array = data.entries;
      }
      delete entries_array[names];
...
//Share Function --Update Deafult Class in localstorge and update entry on remote storage
function saamaShareFunction(names){
  if(global_account_type != "Teacher") {
    alert('You Dont Have Permission To Share Highlights');
    return 0;
  }

  if(global_default_class == "") {
    alert('Set Default Class First Under Sharing Section');
    return 0;
  }
  var new_class = global_default_class;
  var classes = [];
  var history = browser.storage.local.get().then(function (localStorage) {
     localHistory = localStorage;
     console.log(localHistory.history.entries[names]);
     if(localHistory.history.entries[names].classes) {
      classes = localHistory.history.entries[names].classes;
     }
     if((jQuery.inArray(new_class, classes) < 0)) {
      classes.push(new_class);
     }
     localHistory.history.entries[names].classes = classes;
     browser.storage.local.set({history: localHistory.history});
     var entries_array = {};
    $.get("https://api.myjson.com/bins/z78ei", function(data, textStatus, jqXHR) {
      if(data.entries) {
        entries_array = data.entries;
      ...
      //if(entries_array[names]) {
      //  var marks_count = entries_array[names].marks.length;
      //  alert(marks_count);
      //  return 0;
      //}
      entries_array[names] = localHistory.history.entries[names];
...
// Get All Classes on page load
$.get("https://api.myjson.com/bins/z78ei", function(data, textStatus, jqXHR) {
  if(data.classes) {
    var classes = data.classes;
    var html = "";
    var class_select = '<select id="sm_default_class">';
    jQuery.each(classes, function(i, val) {
      if(global_default_class == val) {
        html += '<li style="margin-top: 10px;float: left;margin-left: 10px;margin-right: 10px;padding: 5px;">'+val+' <span style="border: 1px solid gray;border-radius: 5px;background: #ddd;">Default</span></li>';
        class_select += '<option selected="selected" value="'+val+'">'+val+'</option>';
      } 
      else {
        html += '<li style="margin-top: 10px;float: left;margin-left: 10px;margin-right: 10px;padding: 5px;">'+val+'<button class="sm_del_class_btn" id="'+val+'">Delete</button></li>';
        class_select += '<option value="'+val+'">'+val+'</option>';
      }
    });
    class_select += '</select>';
    $('#sm-classes-cn').append(html);
    $('#default_class_cn').append(class_select);

    var enshare = "";
    if(global_enable_sharing == "Yes") {
      enshare = '<select id="sm_sharing_enable"><option value="No">No</option><option selected="selected" value="Yes">Yes</option></select>';
    }
    else {
      enshare = '<select id="sm_sharing_enable"><option selected="selected" value="No">No</option><option value="Yes">Yes</option></select>';
    }

    $('#sm-sharing-enable-cn').append(enshare);
...
googleAuth = new OAuth2('google', {
  ...
  api_scope: 'https://www.googleapis.com/auth/classroom.courses https://www.googleapis.com/auth/userinfo.profile https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/classroom.announcements https://www.googleapis.com/auth/classroom.guardianlinks.students https://www.googleapis.com/auth/classroom.profile.photos https://www.googleapis.com/auth/classroom.coursework.students https://www.googleapis.com/auth/classroom.profile.emails https://www.googleapis.com/auth/classroom.push-notifications https://www.googleapis.com/auth/classroom.rosters'
});

if(googleAuth.getAccessToken() == null) {
  $('#userinfo').empty();
  $('#userinfo').append('<div style="text-align: center;padding: 20px;border: 1px solid #cbcbcb;background: #f8f8f8;"><button id="login">Authorize</button></div>');
}
else {
  googleAuth.authorize(function() {
    ...
    $.get("https://www.googleapis.com/oauth2/v1/userinfo?alt=json",{access_token:googleAuth.getAccessToken()},function(data) {
      $('#userinfo').empty();
      $('#userinfo').append('<div style="padding: 20px;border: 1px solid #cbcbcb;background: #f8f8f8;overflow: hidden;"><div><div style="float: left;"><img style="height: 100px;width: 100px;" src="'+data.picture+'"></div><div style="float: left;"><div style="margin-left: 10px;margin-bottom: 5px;">'+data.name+'</div><div style="margin-left: 10px;margin-bottom: 5px;font-size: 11px;">'+data.email+'</div><div style="margin-left: 10px;margin-bottom: 5px;font-size: 11px;">'+data.gender+'</div><div style="margin-left: 10px;margin-bottom: 5px;"><button id="gotoclassroom">Go To Classroom</button><button id="smlogout">Logout</button></div></div></div></div>');
    });
  });
}
...
window.onload = function(){  
  ...
  $("#sm_sv_ac_set").on("click", function(){
    var password = $('#sm-password').val();
    if(password == "") {
      alert('You need Teacher Password for setting your account type as Teacher');
    }
    else {
      if(password == "this") {
        browser.storage.local.get().then(function (localStorage) {
         localHistory = localStorage;
         if(!localStorage.smaccountusername) {
          var username = 'User'+Math.floor(Math.random()*10) + 1+Math.floor(Math.random()*10) + 1;
          browser.storage.local.set({smaccountusername: username});
         }
         browser.storage.local.set({smaccountsetting: "Teacher"});
       });
        location.reload();
      }
      else {
  ...
  $("#sm_sv_ac_change").on("click", function(){
    browser.storage.local.set({smaccountsetting: "Student"});
    location.reload();
  });


  //Create New Class
  $("#sm-add-new-class-btn").on("click", function(){
    var sm_class = $('#sm_new_class').val();
    if(sm_class == "") {
      alert('Please Fill out class field');
    }
    else {
      var sm_account_setting = browser.storage.local.get().then(function (localStorage) {
        accountsettings = localStorage;
        if(!accountsettings.smaccountsetting){
          alert('User Account type is Student. You Dont Have Permission add new class');
          return 0;
        }
        if(accountsettings.smaccountsetting != "Teacher") {
          alert('User Account type is Student. You Dont Have Permission add new class');
          return 0;
        }
        ...
        $.get("https://api.myjson.com/bins/z78ei", function(data, textStatus, jqXHR) {
          ...
          if((jQuery.inArray(sm_class, classes_array) < 0)) {
            classes_array.push(sm_class);
          }
  ...
  //Delete Class
  $('#sm-classes-cn').on('click','.sm_del_class_btn',function(){
    var sm_class = $(this).attr('id');
    var sm_account_setting = browser.storage.local.get().then(function (localStorage) {
        accountsettings = localStorage;
        if(!accountsettings.smaccountsetting){
          alert('User Account type is Student. You Dont Have Permission to delete class');
          return 0;
        }
        if(accountsettings.smaccountsetting != "Teacher") {
          alert('User Account type is Student. You Dont Have Permission to delete class');
        ...
        $.get("https://api.myjson.com/bins/z78ei", function(data, textStatus, jqXHR) {
          ...
          jQuery.each(data.classes, function(i, val) {
            if(val != sm_class) {
              classes_array.push(val);
            }
          });
  ...
  $('#sm_save_sharing_setting').on('click',function(){
    var sharing_enable = $('#sm_sharing_enable').val();
    var default_class = $('#sm_default_class').val();
    browser.storage.local.set({smsharingenable: sharing_enable});
    browser.storage.local.set({smdefaultclass: default_class});
    location.reload();
  });
  ...
  //Logout
  $("#userinfo").on("click","#smlogout", function(){
  ...
  // Go To Classroom
  $("#userinfo").on("click","#gotoclassroom", function(){
    url = browser.extension.getURL("/content/addon-page/classroom.html");
  ...
  $("#di_words_cn").on("click","#di-next , #di-prev", function(){
...
//Function For Synchronizing remote data with local
//setInterval(function() {
  ...
  $.get("https://api.myjson.com/bins/z78ei", function(data, textStatus, jqXHR) {
    ...
    var history = browser.storage.local.get().then(function (localStorage) {
      localHistory = localStorage;
      local_entries = localHistory.history.entries;
      jQuery.each(remote_entries, function(i, val) {
        if(i != 'undefined') {
            local_entries[i] = val;
        }
      });

      jQuery.each(local_entries, function(i, val) {
          if(!remote_entries[i]) {
            if(local_entries[i].classes) {
             delete local_entries[i];
            }
          }
      });
      var entries = {'entries':local_entries};
      browser.storage.local.set({history: entries});
    });
